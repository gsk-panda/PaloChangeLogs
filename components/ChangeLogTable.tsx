  ...
  useEffect(() => {
    if (expandedId) {
        ...
        if (record && !detailedDiffs[record.id] && !loadingDetails[record.id]) {
            const hasDetailData = record.diffBefore && record.diffBefore !== 'No previous configuration state.' && 
                                  record.diffAfter && record.diffAfter !== 'No new configuration state.';
            
            if (hasDetailData) {
                console.log(`[ChangeLogTable] Using existing detail data for seqno ${record.seqno} from database`);
                setDetailedDiffs(prev => ({ 
                    ...prev, 
                    [record.id]: { 
                        before: record.diffBefore || '', 
                        after: record.diffAfter || '' 
                    } 
                }));
            } else {
                console.log(`[ChangeLogTable] Fetching detail data for seqno ${record.seqno} from Panorama`);
